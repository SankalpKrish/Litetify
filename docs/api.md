...
│  │  librespot.rs — Optional native audio (feature-gated)           │  │
│  └─────────────────────────────────────────────────────────────────┘  │
│                                                                        │
│  ┌────────────────────────────────────────────┐                       │
│  │  auth/ · config.rs · mods/                 │                       │
...
- **Playback engine** (`playback/websdk.rs`) — Uses Tauri's event system (`emit()`) to dispatch playback commands to the frontend's WebSDK player. The optional `librespot` feature provides a native Rust audio backend.
...
| `playback::librespot` | `playback/librespot.rs` | Native audio playback (feature-gated)                       |
...
| `get_config`       | `config::get_config`   | —                                     | `Result<LitetifyConfig>` | Current app config                         |
| `set_config`       | `config::set_config`   | `config: LitetifyConfig`              | `Result<()>`             | Save app config                            |
| `reset_config`     | `config::reset_config` | —                                     | `Result<LitetifyConfig>` | Reset to defaults                          |
...
| `api_create_playlist`      | `api::playlists::api_create_playlist`      | `name, description?, public?`               | `PlaylistDetail`   | Creates new playlist         |
| `api_update_playlist`      | `api::playlists::api_update_playlist`      | `playlist_id, name?, description?, public?` | `SpotifyPlaylist`  | Updates metadata             |
...
| `api_play`                  | `api::player::api_play`                  | `device_id?, uri?, context_uri?, uris?, position_ms?, offset?` | `()`              | Resume or play specific content |
...
### Playback Engine (Librespot — feature-gated, `cfg(feature = "librespot")`)

| Command                    | Rust Path                                       | Args          | Returns      | Notes                           |
| -------------------------- | ----------------------------------------------- | ------------- | ------------ | ------------------------------- |
| `init_librespot`           | `playback::librespot::init_librespot`           | —             | `Result<()>` | Initialize native audio backend |
| `librespot_play`           | `playback::librespot::librespot_play`           | `uri?`        | `Result<()>` | Play via librespot              |
| `librespot_pause`          | `playback::librespot::librespot_pause`          | —             | `Result<()>` | Pause native playback           |
| `librespot_resume`         | `playback::librespot::librespot_resume`         | —             | `Result<()>` | Resume native playback          |
| `librespot_seek`           | `playback::librespot::librespot_seek`           | `position_ms` | `Result<()>` | Seek within track               |
| `librespot_set_volume`     | `playback::librespot::librespot_set_volume`     | `volume`      | `Result<()>` | Set volume (0-100)              |
| `librespot_next`           | `playback::librespot::librespot_next`           | —             | `Result<()>` | Next track                      |
| `librespot_previous`       | `playback::librespot::librespot_previous`       | —             | `Result<()>` | Previous track                  |
| `librespot_toggle_shuffle` | `playback::librespot::librespot_toggle_shuffle` | —             | `Result<()>` | Toggle shuffle                  |

Librespot commands are only compiled when the `librespot` Cargo feature is enabled (not default). They provide a native Rust audio playback backend using the `librespot` crate (rodio-backend, rustls-tls-webpki-roots).
